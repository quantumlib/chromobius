...
def _patch_svg_viewer_helper_single_patch_wraparound_clip(
    *,
    ...
    out_lines.append(f'''    <path d="M{tl.real},{tl.imag} L{br.real},{tl.imag} L{br.real},{br.imag} L{tl.real},{br.imag} Z" />''')
...
def patch_svg_viewer(
    ...
    *,
    ...
    helper = _patch_svg_viewer_helper_single_patch_wraparound_clip if wraparound_clip else _patch_svg_viewer_helper_single_patch
    ...
    for outline_index, outline in enumerate(patches):
        if wraparound_clip and len(outline.without_wraparound_tiles().tiles) < len(outline.tiles):
